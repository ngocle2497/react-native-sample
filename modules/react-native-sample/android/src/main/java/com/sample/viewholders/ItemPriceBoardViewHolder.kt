...
class ItemPriceBoardViewHolder(itemView: View) : ViewHolderWrapper<Quote>(itemView) {
            ...
            UiThreadUtil.runOnUiThread {
                run {
                    val priceColorAnimation =
                        ValueAnimator.ofObject(ArgbEvaluator(), ColorName.textPrimary.color, tintColor)
                    priceColorAnimation.duration = 700
                    priceColorAnimation.addUpdateListener { animator -> tvTradePrice.setTextColor(animator.animatedValue as Int) }
                    ...
                    overlayColorAnimation.addUpdateListener { animator -> cvContainerPrice.setCardBackgroundColor(animator.animatedValue as Int) }
